		...
		switch (config->GetMarker_All_KindBC(iMarker)) {
            ...
            case RIEMANN_BOUNDARY:
            	if(MainSolver == FLOW_SOL) {
                	if ( config->GetMarker_All_TagBound(iMarker) == "symmetry1" || config->GetMarker_All_TagBound(iMarker) == "symmetry2" ) {
                		solver_container[MainSolver]->BC_Mixing_Riemann(geometry, solver_container, numerics[CONV_BOUND_TERM], numerics[VISC_BOUND_TERM], config, iMarker);
                	}
                	else {
