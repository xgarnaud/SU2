...
void COutput::SetRestart_HDF5(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  // htri_t ex =  H5Lexists( file_id, timename.c_str(), H5P_DEFAULT);
  // hid_t time_id = H5Gcreate (file_id, timename.c_str(), H5P_DEFAULT ,H5P_DEFAULT ,H5P_DEFAULT );
  
  // H5LTmake_dataset(ele_id,"xyz",2,dims,H5T_NATIVE_DOUBLE,xyz.get_ptr_cpu());
  H5Gclose(ele_id);
  H5Fclose(file_id);
